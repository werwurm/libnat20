    ...
    switch (request.request_type) {
        ...
        case n20_msg_request_type_issue_eca_ee_cert_e: {
            ...
            while ((opt = getopt_long(argc, argv, "k:n:o:p:f:N:u:l:?h", long_options, NULL)) !=
                ...
                switch (opt) {
                    case 'k':
                        request.payload.issue_eca_ee_cert.key_type = parse_key_type(optarg);
                        break;
                    case 'n':
                    ...
                    case 'o':
                        ...
                        break;
                    case 'p':
                        request.payload.issue_eca_ee_cert.parent_key_type = parse_key_type(optarg);
                        break;
                    case 'f':
        ...
        case n20_msg_request_type_eca_ee_sign_e: {
            ...
            while ((opt = getopt_long(argc, argv, "k:n:o:N:u:M:?h", long_options, NULL)) != -1) {
                switch (opt) {
                    case 'r':
                        ...
                        break;
                    case 'p':
                        request.payload.eca_ee_sign.parent_key_type = parse_key_type(optarg);
                        break;
                    case 'k':
                        request.payload.eca_ee_sign.key_type = parse_key_type(optarg);
                        break;
                    case 'n':
            ...
            if (request.payload.eca_ee_sign.parent_key_type == n20_crypto_key_type_none_e) {
                fprintf(stderr, "Invalid parent key type\n");
                print_usage(argv[0]);
                exit(EXIT_FAILURE);
            }

            if (request.payload.eca_ee_sign.key_type == n20_crypto_key_type_none_e) {
                fprintf(stderr, "Invalid key type\n");
                print_usage(argv[0]);
                exit(EXIT_FAILURE);
            }
